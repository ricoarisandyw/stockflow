...
export function FormInvoice({
  ...
  const invoiceDetailQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await apiClient.GET("/invoices/{id}", {
  ...
  const preview = useMemo(() => {
    const lines = watchedItems
      ...
      .filter(
        (line): line is { unitPrice: number; quantity: number } =>
          line !== null,
      );
    ...
    onError: (error) => {
      ...
      details?.forEach((d) => {
        if (d.field && d.message)
          setError(d.field as keyof TFormInput, { message: d.message });
      });
      setError("root", {
        message:
  ...
  return (
    <>
      {isViewing && invoiceDetailQuery.data?.data && (
        <InvoicePrintView invoice={invoiceDetailQuery.data.data} />
      )}

      <form
        onSubmit={
          isViewing ? (e) => e.preventDefault() : handleSubmit(onSubmit)
        }
        className="space-y-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm print:hidden"
      >
                ...
                <div key={field.id} className="flex items-start gap-2">
                  <div className="flex-1">
                    <select
                      disabled={isViewing}
                      aria-invalid={
                        errors.items?.[index]?.productId ? true : undefined
                      }
                  ...
                  <div className="w-28">
                    <input
                      ...
                      disabled={isViewing}
                      aria-invalid={
                        errors.items?.[index]?.quantity ? true : undefined
                      }
                      className="block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-gray-500 focus:outline-none focus:ring-1 focus:ring-gray-500 disabled:bg-gray-100 disabled:text-gray-500"
